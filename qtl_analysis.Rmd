...
MQM_ion <- mqmaugment(MQM_ion_crossobject, minprob = 0.1)

# Set automatic cofactors across all genome (100)
MQM_ion_cofactors <- mqmautocofactors(MQM_ion, 100)

# Multiqtl scan using MQM
MQM_ion_scan <- MQM_ion_pheno.col %>%
  mutate(
    mqmscan = 
      map(.x = pheno.col,
          .f = ~ mqmscan(
            cross = MQM_ion, 
            cofactors = MQM_ion_cofactors,
            pheno.col = .x, 
            step.size = 1,
            window.size = 25,
            n.clusters = 4)))
...
MQM_ion_scan_perms <- MQM_ion_scan %>%
...
MQM_ion_scan_perms2 <- MQM_ion_scan_perms %>%
  mutate(perms_2 = purrr::map(.x = perms, .f = ~ mqmprocesspermutation(.x)))


MQM_ion_hun_summary <- MQM_ion_scan_perms2 %>%
  ...
  rename(p_ci.low = p_pos_l, p_ci.high = p_pos_h,
         treshold = treshold) %>%
  unnest(lod) %>%
  mutate(chr = as.integer(chr))

MQM_ion_LOD_map <- MQM_ion_scan_perms2 %>%
  mutate(
    mapa = map(
      .x = mqmscan,
      .f = ~ mapa_base_MQM(
        SCANONE = .x, 
        TIBBLE = MQM_ion_hun_summary))) %>%
  .$mapa %>%
  Reduce(bind_rows, .)
...
MQM_ion_hun_add_lodplots <- MQM_ion_LOD_map %>%
  group_by(trait) %>%
  nest %>%
