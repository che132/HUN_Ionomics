...
phen_cs <- scale_colour_manual(values = phenotypes_color_scheme,
                               drop = T)
...
ion_chrom_plot <- chr_plot_base +
  ...
  scale_color_manual(values = phenotypes_color_scheme,
                     drop = T, 
...
ion_effectplot_df_cov2 <- ion_effectplot_df_cov %>%
  ...
  mutate(epdf = 
           map(.x = 
                 ep,
               .f = ~ map_df(.x =., .f = ~ as_tibble(.x, rownames = "hun"), .id = "what") %>%
...
rm(ion_effectplot_df_cov)

  mutate(x = as.double(hun),
         x = jitter(x, 0.1))
                    ))
