...
ion_int_add_cov_lodplot_df <- lodplot_map_df(
  SCANONE = ion_crossobject_int_addcov.out.hk,
  SUMMARY = scanone_summary_int_add_cov,
  PERMS = ion_crossobject_int_addcov.operm,
  alpha = 0.1,
  ci.funciton = "lodint_add"
  )
    ...
    plot = map2(
      ...
      .f = ~ .x %>% 
        ...
        ggplot(
          ...
          aes(x = cM, 
              y = lod, 
              group = int_adderaction(chr, alpha)
          )
          ) +
            geom_line(size = 0.75, alpha = 0.3) +
          geom_line(
          ...
          geom_hline( 
            aes(yint_addercept = unique(threshold)),
            color = "red",
            linetype = 2
            ) +
          scale_alpha_identity() +
...
ion_int_add_cov_lodplots_signif_single <- ion_int_add_cov_lodplots_signif %>%
  select(phenotype, data) %>%
  unnest(data) %>% {
    ggplot(data =., 
           aes(x = cM, y = lod, group = int_adderaction(chr, alpha))) +
      geom_line(size = 0.75, alpha = 0.3) +
      geom_line(aes(alpha = alpha), size = 0.75) +
      geom_ribbon(aes(ymin = 0, ymax = y), fill = "red", alpha = 0.8 ) +
      geom_hline(aes(yint_addercept = threshold), color = "red",
                 linetype = 2) +
      scale_alpha_identity() +
      scale_x_continuous(name = "chr", 
                         breaks = unique(.$breaks),
...
ggsave(
   filename =
     "output/plots/scanone_lodplots/ion_int_add_cov_lodplots_signif_single.pdf", 
   plot = ion_int_add_cov_lodplots_signif_single, 
   width = 15, 
   height = 18
)
