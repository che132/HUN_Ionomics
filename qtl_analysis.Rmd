...
ion_WT_lodplots_signif <- ion_WT_lodplots_all %>%
  semi_join(ion_WT_lodplots_signif) %>%
  select(phenotype, data) %>%
  unnest(data) %>% {
    ggplot(data =., 
           aes(x = cM, y = lod, group = interaction(chr, alpha))) +
...
qtl_detected_all <- bind_rows(
  ...
  mutate(model = ifelse(!is.na(sufix), paste0(model, ".", sufix), model)) %>%
...
model_line_scheme <- c("HUN" = "dotdash", "add" = "F1", "int" = "dotted", "WT" = "longdash",
                       "addint.i" = "solid", "addint.f" = "twodash")

phenotypes_color_scheme_df <- ion_pheno.col %>% 
  separate(phenotype, into = c("ion", "tissue")) %>%
  mutate(tissue = factor(tissue, levels = c("leaf", "seed", "ratio"))) %>%
  arrange(ion, tissue) %>% 
  mutate(phenotype = paste0(ion, "_", tissue)) %>%
  mutate(color = parula(60)) %>%
  mutate(phenotype = as_factor(phenotype))

phenotypes_color_scheme <- phenotypes_color_scheme_df$color
names(phenotypes_color_scheme) <- levels(phenotypes_color_scheme_df$phenotype)

phen_cs <- scale_colour_manual(values = phenotypes_color_scheme, drop = T)

chr_plot_base <- qtl_detected_all_lodplot %>%
  unnest(data) %>%
  ...
  pivot_longer(-chr) %>%
  select(-name) %>%
  ggplot(data =., aes(x = chr, y = value)) +
  geom_line(size = 1.5) + 
  scale_y_continuous(trans = "reverse")

chr_plot_phenotypes_df <- qtl_detected_all_lodplot %>%
  unnest(data) %>%
  ungroup %>%
  mutate(chr = as_factor(chr)) %>%
  ...
  mutate(x = as.double(chr) + 0.05 * comp * id2) %>%
  select(-c(id:id2)) %>%
  ungroup %>%
  pivot_longer(contains("ci")) %>%
  mutate(phenotype = factor(phenotype, levels = levels(phenotypes_color_scheme_df$phenotype)))


ion_chrom_plot <- chr_plot_base +
  geom_line(data = chr_plot_phenotypes_df, 
         aes(x = x, 
             y = value, 
             group = interaction(phenotype, model),
             color = phenotype,
             linetype = model),
         size = 1.25) +
  scale_linetype_manual(values = model_line_scheme) +
  scale_color_manual(values = phenotypes_color_scheme, drop = T, breaks = chr_plot_phenotypes_df$phenotype %>% unique %>% sort) +
...
# Effect plots forn hun and wt (without hun cov interaction)
ion_effectplot_df_HUN_WT <- ion_effectplot_df_pre %>%
  filter(model %in% c("WT", "HUN")) %>%
  mutate(ep = pmap(
    .l = .,
    .f = ~ with( 
      list(...),
...
genotypes_scheme_color <- c("CML312" = "#9a3a28", "HET" = "#581d2c",
                            "W22" = "#2e1342")


ion_effectplot_df_HUN_WT %>%
  select(ion, tissue, qtl, model, ep) %>%
  filter(ion == "S" & qtl == "1@51.5") %>%
  mutate(ep_df = map( 
    .x = ep,
  ...
  geom_errorbar(aes(ymin = Means - SEs , ymax = Means + SEs),
                width = 0.2, size = 1) +
  scale_color_manual(values = genotypes_scheme_color) +
  xlab(NULL) +
...
ion_qtl_granges_example <- qtl_detected_all %>%
  select(seqnames = chr, start = ci.low, end = ci.high, phenotype) %>%
  mutate(strand = "*") %>%
  as_granges()

ion_qtl_overlaps <-join_overlap_self(ion_qtl_granges_example) %>%
  as_tibble %>%
  distinct %>% 
  filter(phenotype != phenotype.overlap)
...
exons_bed <- system.file("extdata", "exons.bed",
                         package = "HelloRangesData")

build <- system.file("extdata", "hg19.genome",
                     package = "HelloRangesData") %>% 
  read.delim(header = FALSE, col.names = c("seqnames", "width")) %>%
  as_granges(start = 1) %>%
  set_genome_info(genome = "hg19", seqlengths = width(.))

exons <- read_bed(exons_bed, genome_info = build)

cpg_bed <- system.file("extdata", "cpg.bed", package = "HelloRangesData")

cpg <- read_bed(cpg_bed, genome_info = build)
