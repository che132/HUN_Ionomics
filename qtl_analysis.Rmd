...
>>>>>>> cf49913e98ab0965feec295ca504befc883349b6
...
qtl_full_model_results <- qtl_models_all %>%
  qtl_fullmodel_df(.)
...
qtl_drop_one_results

qtl_models_all %>%
  qtl_dropone_df(.) %>%
  filter(if_any(c(p.F, s.F), ~ grepl("N\\.S\\.|^\\*$", .))) %>%
  select(phenotype:qtl, LOD, `%var`, p.F, s.F)

qtl_full_model_results %>%
  filter(if_any(c(p.F, s.F), ~ grepl("N\\.S\\.|^\\*$", .))) %>%
  select(phenotype:term, LOD, `%var`, s.Chi2, s.F)

Reduce(
      setdiff,
      strsplit(c(texto, texto2), split = " ")
      )
```
